...
public class LightProblem {
    ...
    /**
     * 不在当前位置做决定，在前一个位置做决定，默认前前位置之前都是1
     * <p>
     * 只有0位置能决策，其他位置都受前前位置决定
     *
     * @param arr
    ...
    /**
     * 当我调用process2，一定要保证，传入的index >= 3（直想做平凡位置，0位置和1位置比较特殊，提前处理好）
     * <p>
     * 当前来到index位置，不在index上做决定
     *
     * @param prepre      前前灯的状态，
     * @param pre         前灯的状态，这是要做决定的灯
     * @param endStatus   N-1号灯的状态
     * @param firstStatus 0号灯的状态
     * @return 让所有灯都亮，最少要按几次开关
     **/
    public static int process2(int[] arr, int index, int prepre, int pre, int endStatus, int firstStatus) {
        ...
        // 目前在N-2号灯上做决定
        // 因为N-1位置的灯，有可能已经被0位置改了，所以不能直接使用arr[index]，需要使用endStatus
        else {
